...
- Playback engine: client-side deterministic replay of the ghost's deltas (server sends the ghost's revealed-state timeline once at start — ghosts have no hidden info to protect since their run is public).
  - **Correction (2026-08-26):** the "run is public, so nothing needs protecting" premise is **not accurate against the shipped code**. `action_runs.action_log` records per-verb `target` (host id, IP, username, or party id) — a field the existing unauthenticated `/r/{token}` DTO deliberately strips for leak-safety (PR #41), not because it was unused. Daily Breach compiles every player that day onto the same world via `_deterministic_daily_seed`; showing a ghost's exact targets during a live race is a real-time spoiler on that shared map/IOC layout, not spectator color. **Build requirement:** ghost playback must use a server-controlled DTO, never a raw `action_log` or live `state.delta` passthrough. For Daily (shared-seed) races: map-state-only animation (known/unknown hosts, isolated/compromised, timing) — never raw targets. For scenario "race this run" (opt-in same seed, not shared live Daily pressure): fuller detail is lower-risk and may be acceptable, but still must exclude `warranted`, `correct`, `on_attack_path`, IOC bodies, and `seed` — same exclusions as the public DTO. Treat this as its own leak-safety design pass before implementation, with the same rigor as the `/r/{token}` DTO work (PR #41) and the dossier lock-boundary fix (PR #34), not as a simple data-passthrough feature.
