...
/**
 * Everything below LandingPage.tsx's hero — social proof, how-it-works,
 * scenario library, multiplayer callout, pricing, final CTA, footer.
 * Extracted so TeaserLandingPage.tsx (Phase 1) can render the same
 * marketing content below its own teaser hero without duplicating ~250
 * lines of JSX. LandingPage.tsx renders this unchanged, at the same DOM
 * position it always has — behaviorally identical to before the extraction.
 *
 * The "Pricing" section below carries `id="pricing"` instead of taking a
 * ref, so any nav (this component's own, or a page composing it, like
 * TeaserLandingPage) can link to it with a plain
 * `document.getElementById("pricing")?.scrollIntoView(...)` instead of
 * prop-drilling a ref across component boundaries.
 */
